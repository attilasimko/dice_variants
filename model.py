...
from tensorflow.keras.layers import Input, Conv2D, BatchNormalization, Activation, Concatenate, ReLU, Conv2DTranspose
...
def unet_2d(input_shape, num_filters, num_classes, batchnorm=False):
    ...
    x = Conv2D(num_filters * 8, kernel_size=2, padding='same', kernel_initializer=initializer)(x)
    x = Concatenate()([x, x_5])
    if (batchnorm):
        x = BatchNormalization()(x)
    x = ReLU()(x)
    ...
    # x = UpSampling2D(size=(2, 2))(x)
    x = Conv2DTranspose(x.shape[-1], (3,3), strides=(2,2), padding='same')(x)
    x = Conv2D(num_filters * 4, kernel_size=2, padding='same', kernel_initializer=initializer)(x)
    x = Concatenate()([x, x_3])
    if (batchnorm):
        x = BatchNormalization()(x)
    x = ReLU()(x)
    ...
    # x = UpSampling2D(size=(2, 2))(x)
    x = Conv2DTranspose(x.shape[-1], (3,3), strides=(2,2), padding='same')(x)
    x = Conv2D(num_filters * 2, kernel_size=2, padding='same', kernel_initializer=initializer)(x)
    x = Concatenate()([x, x_2])
    if (batchnorm):
        x = BatchNormalization()(x)
    x = ReLU()(x)
    ...
    # x = UpSampling2D(size=(2, 2))(x)
    x = Conv2DTranspose(x.shape[-1], (3,3), strides=(2,2), padding='same')(x)
    x = Conv2D(num_filters, kernel_size=2, padding='same', kernel_initializer=initializer)(x)
    x = Concatenate()([x, x_1])
    x = Conv2D(num_filters, kernel_size=3, padding='same', kernel_initializer=initializer)(x)
