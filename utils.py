        ...
        for slc in range(y_true.shape[0]):
            for i in range(y_true.shape[3]):
                ...
                if (replace_alphas[i]):
                    ...
                    alpha = tf.stop_gradient(tf.cast(tf.cast(tf.not_equal(tf.argmax(y_pred[slc, :, :, :], -1), i), tf.float64) * 2 / (K.sum(flat_true) + epsilon), tf.float64)) # 2 / U
                else:
                    alpha = float(alphas[i])

                if (replace_betas[i]):
                    # beta = tf.stop_gradient(coin_coef_b(y_true[slc, :, :, i], y_pred[slc, :, :, i], epsilon))
                    beta = tf.stop_gradient(tf.cast(2 * I / (U * U), tf.float64)) # 2 * I / (U * U)
                else:
                    beta = float(betas[i])
