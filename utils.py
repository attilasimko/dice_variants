...
def coin_loss(_alphas, _betas, epsilon):
    ...
    def loss_fn(y_true, y_pred):
        ...
        for slc in range(y_true.shape[0]):
            ...
            for i in range(y_true.shape[3]):
                if (replace_alphas[i]):
                    alpha = tf.stop_gradient(coin_coef_a(y_true[slc, :, :, i], y_pred[slc, :, :, i], epsilon)) * -tf.stop_gradient(tf.math.log(tf.where(tf.equal(y_pred[slc, :, :, i], 0.0), np.finfo(float), y_pred[slc, :, :, i])))
                else:
