...
def coin_loss(_alphas, _betas, epsilon):
    ...
    def loss_fn(y_true, y_pred):
        ...
        for i in range(y_true.shape[3]):
            ...
            # if (replace_betas[i]):
            beta = 0.0 # tf.stop_gradient(coin_coef_b(y_true[:, :, :, i], y_pred[:, :, :, i], epsilon))
            # else:
            ...
            loss += (1 + K.sum(alpha * y_true[:, :, :, i] * tf.stop_gradient(tf.where(tf.equal(tf.argmax(y_pred, 3), i), tf.zeros_like(y_pred[:, :, :, i]), tf.ones_like(y_pred[:, :, :, i]))) * y_pred[:, :, :, i] + beta * y_pred[:, :, :, i]))
            iter += 1
        return loss
    return loss_fn
