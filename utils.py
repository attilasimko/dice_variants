    ...
    def loss_fn(y_true, y_pred):
        ...
        for slc in range(y_true.shape[0]):
            for i in range(y_true.shape[3]):
                if (replace_alphas[i]):
                    alpha = tf.stop_gradient(2 / coin_U(K.flatten(y_true[slc, :, :, i]), K.flatten(y_true[slc, :, :, i]))) # coin_coef_a(y_true[slc, :, :, i], y_pred[slc, :, :, i], epsilon))
                else:
...
def evaluate(experiment, gen, model, name, labels, epoch):
    ...
    for j in range(len(labels)):
        ...
        experiment.log_metrics({f'{name}_dice_{labels[j]}': np.mean(metric_dice[j]),
                                ...
                                f'{name}_dice_a_{labels[j]}': np.sum(metric_dice_a[j]),
                                f'{name}_dice_a_{labels[j]}_std': np.std(metric_dice_a[j]),
                                f'{name}_dice_b_{labels[j]}': np.sum(metric_dice_b[j]),
                                f'{name}_dice_b_{labels[j]}_std': np.std(metric_dice_b[j]),
                                f'{name}_u_{labels[j]}': np.sum(metric_u[j]),
                                f'{name}_i_{labels[j]}': np.sum(metric_i[j]),
    ...
    return (np.reshape(np.hstack(grads).T, (-1)), [coin_a(np.min(u)) for (u, i) in zip(metric_u, metric_i)], [coin_b(np.mean(u), np.mean(i)) for (u, i) in zip(metric_u, metric_i)])